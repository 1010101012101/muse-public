...
def findSourceTargets(dConfig):
    ...
    for tProjectRow in lProjectRows:
        ...
        ## skip projects which are not in the filtered cyber projects file listing
        if sProjectName not in dConfig['cyber-projects']: continue
...
def findBuildTargets(dConfig):
    ...
    for sProjectName in dConfig['cyber-projects']:
...
###
def usage():
    warning('Usage: queueProjects_cyber.py --corpus-dir-path=/data/corpus_0to7 --cyber-projects-file=./cyberPhysicalBuiltTest.txt --forks=5 --analyze-projects --crawl-projects --unbuilt-projects-only --queue-projects --debug')
...
def main(argv):
    ...
    sCyberProjectsFile = 'cyberPhysicalBuiltTest.txt'
    ...
    #dConfig['es-file-index-name'] = 'cyber-corpus-source'
    ...
    options, remainder = getopt.getopt(sys.argv[1:], 'c:y:f:apuqs:d', ['corpus-dir-path=','cyber-projects-file=','forks=','analyze-projects','crawl-projects','unbuilt-projects-only','queue-projects','queue-site=','debug'])
    ...
    for opt, arg in options:
        ...
        elif opt in ('-y', '--cyber-projects-file'):

            sCyberProjectsFile = arg
    ...
    if dConfig['analyze-projects'] and not os.path.isfile(sCyberProjectsFile): bError = True
    ...
    if bError: usage()
        ...
        with open(sCyberProjectsFile, 'r') as fCyberProjectsFile:
        
            for sProjectPath in fCyberProjectsFile:

               dConfig['cyber-projects'].append( os.path.basename( sProjectPath.strip() ) )

        if dConfig['debug']: debug(json.dumps(dConfig['cyber-projects'], indent=4))
