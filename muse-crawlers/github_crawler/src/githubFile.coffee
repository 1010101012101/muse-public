...
##
#  Github Crawler which takes a file containing a list of projects in the form
#    <author>/<project_name>
#  will check to ensure only new projects are downloaded (checks corpus volumes)
...
argv = yargs
    .usage('Usage: $0 --token 377sdgsr32r3afpafsp ' +
    	...
    	'--out /datastore' +
        '--filename file.txt')
    .demand(['token','mongoDb', 'mongoCollection', 'out', 'filename'])
    .argv;
...
projects = argv.filename
github = octonode.client token
myLines = fs.readFileSync(projects).toString().match(/^.+$/gm)
count = 1;

process.title = "github-file-crawler";
...
start = (mongo_collection, redis ) =>
	console.log ''
	console.log 'Starting GitHub File Crawl...'
	console.log ''

	# for each project in file
	async.eachSeries myLines, (line, callback) => 

		gitrepo = github.repo(line)
		gitrepo.info (err, data, headers) =>
			if err
				...
				repo = data
				console.log '---------------------------------------'
				console.log 'Working on #' + count++ + ' of ' + myLines.length
